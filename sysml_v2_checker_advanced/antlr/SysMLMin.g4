...
// `attribute fuelEfficiency redefines fuelEfficiency=.6;`・`:>>Vehicle::position=.98 [m];`
// （SysML v2 Spec Annex A SimpleVehicleModel.sysml:1173,1182,1260,1277,1280,1294）の
// ように、整数部を省いた小数リテラルもある。整数部を `[0-9]*` にして受理する
// （2026-09-04、比較レポートv2 §v2-6 G4。参照実装は同ファイル全体に対して
// 診断0件＝この記法を受理する）。
//
// 範囲式との衝突は起きない: `1..2` の `..` は暗黙リテラルトークンとして
// REAL_LITERAL より優先され、かつ位置`..2`で REAL_LITERAL を試しても
// `[0-9]*`（空）→`'.'`→`[0-9]+` が2文字目の`.`で失敗するため、`..` が
// 単独トークンとして正しく切り出される。`x.y` も同様に `[0-9]+` が失敗する。
REAL_LITERAL
    : [0-9]* '.' [0-9]+ ([eE] [+-]? [0-9]+)?
    ;
...
// `//`の直後に`*`が続く形（`//* ... */`）は、公式xpectフィクスチャの
// 複数行アノテーション規約（`//* XPECT errors --- "..." --- */`、
// Connector_Invalid.sysml等）だけでなく、公式トレーニング例の省略記号
// （`attribute cityScenario : WayPoint[*] = ( //* ... */ );`、
// Analysis Case Usage Example.sysml:10-11）にも使われる。
//
// 参照実装(jar 0.61.0)に問い合わせて確認した挙動（2026-09-04）:
//   - `//* ... */`（同一行で閉じ、後続にコードがある）→ 受理。つまり
//     `*/`までがコメントで、その後ろのコードは生きている。
//   - `//* ...`（`*/`が無い）→ 受理。行コメントとして扱われる。
//   - `/* ... */` を式の中に置く形 → **拒否**。SysML v2では`/* */`は
//     doc/comment本体のトークン（下のDOC_COMMENT）であり、読み飛ばされる
//     コメントではない。ローカルの設計と一致している。
// つまり`//*`は「閉じ`*/`があればブロック、無ければ行コメント」。
//
// 【2026-09-04修正】以前ここには「ANTLRは最長一致を優先するため、`*/`が
// 実在すれば自然にブロック側が選ばれる」と書いてあったが**誤り**だった。
// `//* ... */ );` のように同一行で閉じて後続にコードがある場合、
// `LINE_COMMENT`（行末までの13文字）の方が`//* ... */`（10文字）より
// 長いため、最長一致で行コメントが勝って`);`まで飲み込んでしまう。
// 実際にAnalysis Case Usage Example.sysmlが2箇所でこれに当たり、
// 閉じ括弧が消えて次行の`attribute`でパースエラーになっていた
// （比較レポートv2 §v2-4 E2）。
//
// 対策として、行コメント側の`//*`分岐が`*/`を跨がないようにする。
// これで同一行に`*/`があるときはブロック側が必ず長くなり、無いときは
// 行コメント側が行末まで取れる。コーパス730件の`//*`出現34箇所
// （複数行32・同一行閉じ2・閉じ無し0）すべてで期待どおりに振る舞う。
SLASH_STAR_BLOCK_COMMENT
    : '//*' .*? '*/' -> skip
    ;

// 本文は3通り: (a) `//`直後が`*`以外の通常の行コメント、(b) `//*`で始まり
// 同一行に閉じ`*/`が無い行コメント、(c) 本文が空の`//`単独。(b)の
// `'*' ~[/\r\n]` により`*/`の直前で必ず停止するので、`*/`があるときは
// 上のSLASH_STAR_BLOCK_COMMENTの方が長くなり、最長一致でそちらが選ばれる。
// `-> skip`はルール末尾に1つだけ置く（代替ごとに付けると再生成が
// 「cannot create implicit token for string literal in non-combined grammar」で
// 失敗する。2026-09-04に実際に踏んだ）。
LINE_COMMENT
    : '//' ( ~[*\r\n] ~[\r\n]* | '*' (~[*\r\n] | '*' ~[/\r\n])* )? -> skip
    ;
