...
## v5-8. 【2026-09-25 追記】フェーズ2の再評価

v5 の修正と、MCP の lint ツールの指摘に行番号を付けた修正（3c568f4）の後に、フェーズ2の
評価を同じ設定（gpt-5.6-terra、reasoning=low、3条件 × 8課題 × 1回）でもう一度回した
（`SysMLv2_pilot/sysml-mcp-server/eval/phase2/results/rerun-20260925/`）。

| 条件 | 前回の合格 | 今回の合格 | 平均エラー件数（前回 → 今回） |
|---|---:|---:|---:|
| none（ツール無し） | 2/8 | 2/8 | 11.1 → 8.9 |
| **self（このチェッカーのみ）** | **2/8** | **7/8** | **9.5 → 0.6** |
| self+ref（このチェッカー ＋ 参照実装） | 7/8 | 7/8 | 0.25 → 0.0 |

**このチェッカーだけを渡した条件が、参照実装を渡した条件と同じ合格数になった。**
ツール無しの条件は 2/8 のままなので、LLM のばらつきで説明できる差ではない。前回 self で
不合格だった5課題（t01・t04・t06・t07・t08）がすべて合格に変わった。たとえば t01 では、
1回目の lint が「`kg` は SI にあります（`private import SI::*;` を追加するか `SI::kg` と書く）」
を行番号付きで返し、LLM はそれを直して合格した。各条件1回ずつなので、差にはばらつきも含まれる。

残った不合格は2件:

- **self の t03（エラー5件）: このチェッカーの見逃しが1つ残っていた。** LLM が
  `if sugarNeeded { then addSugar; } else { then pourCoffee; }` と書き、チェッカーは0件と返し、
  LLM はそこで終えた（前回と同じ形の誤った完了）。参照実装は構文エラーにする。
  → 直した（bcd8d4c、Blackboard e9）。本体の先頭の `then X;` を error にし、if の本体の
  succession が外側の要素を直接指す形も「Must be an accessible feature」として報告する。
  境界は `tests/fixtures/reference_conformance/i*.sysml` の16件で測った。コーパス730件では
  どちらの規則も発火せず、誤検出は増えていない。
- **self+ref の t03（モデル無し）**: このチェッカーとは無関係。参照実装の validate を9回呼び、
  往復の上限（10回）で最終モデルを出さずに終わった。

あわせて、前回と今回の LLM のモデル47件を参照実装と突き合わせた。一致しないのは1件だけで、
ツール無し条件の t06 だった。`requirement r : R { subject = drone; }` で subject を束縛した
要求に、`satisfy r by drone;` でもう一度束縛している形で、参照実装は
