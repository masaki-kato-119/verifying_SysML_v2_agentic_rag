"""
ConceptCandidate正規化モジュール（英語対応、仕様書 5.3）
"""
import re
...
class NormalizerEN:
    """
    ConceptCandidate正規化器（英語）
    
    英語依存の処理を行う
    """
    
    def __init__(self):
        self.abstract_nouns = config.ABSTRACT_NOUNS_EN
        self.event_suffixes = config.EVENT_SUFFIXES_EN
    
    def _normalize_pos(self, pos: str) -> POS:
        """
        品詞を正規化（Penn Treebank形式からPOS enumへ）
        - 名詞 → NOUN
        - 動詞 → VERB
        - 固有名詞 → PROPN
        - 形容詞 → ADJ
        """
        # Penn Treebank形式の判定
        if pos in ['NNP', 'NNPS']:  # 固有名詞
            return POS.PROPN
        elif pos.startswith('N'):  # 名詞
            return POS.NOUN
        elif pos.startswith('V'):  # 動詞
            return POS.VERB
        elif pos.startswith('J'):  # 形容詞（JJ, JJR, JJS）
            return POS.ADJ
        else:
            return POS.OTHER
    ...
    def _is_abstract(self, lemma: str, pos: str) -> bool:
        ...
        """
        lemma_lower = lemma.lower()
        if lemma_lower in self.abstract_nouns:
            return True
        
        # 上位語を必要とする名詞パターン
        abstract_patterns = ["type", "method", "concept", "approach", "means", "purpose"]
        for pattern in abstract_patterns:
            if pattern in lemma_lower:
                return True
